...
resource "aws_security_group" "web_sg" {
  name        = "${var.project}-${var.environment}-web-sg" # セキュリティグループ名
  description = "web front role security group"            # 説明
  vpc_id      = aws_vpc.vpc.id                             #別ファイルでも同一ディレクトリの場合は読み込み可能
...
resource "aws_security_group" "app_sg" {
  name        = "${var.project}-${var.environment}-app-sg" # セキュリティグループ名
  description = "app security group"                       # 説明
